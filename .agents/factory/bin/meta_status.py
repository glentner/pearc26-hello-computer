...
"""Print the harness-feedback findings of the repo-root META.md as JSON.

Part of the paper factory's self-improvement loop. The lifecycle skills append
markdown ``## F<n>`` finding sections to ``META.md`` (silence by default);
``paper-release`` surfaces them and ``/paper-harness`` applies them, reading them
through this script rather than parsing the markdown themselves. Mirrors
``paper_status.py``'s principle: the *script* owns the fragile parsing, the model
executes.
...
Usage:
    python3 .agents/factory/bin/meta_status.py [META.md]
    python3 .agents/factory/bin/meta_status.py META.md --status open
    python3 .agents/factory/bin/meta_status.py META.md --severity high --id F1 F3

A missing file is a valid empty state (``exists: false``, no findings, exit 0) —
a paper that logged no harness friction simply has no META.md.
...
def _parse_args(argv: list[str]) -> argparse.Namespace:
    ap = argparse.ArgumentParser(description="Read harness findings from META.md as JSON.")
    ap.add_argument("path", nargs="?", default="META.md", help="path to META.md (default: repo-root META.md)")
    ...
    ap.add_argument("--summary", action="store_true",
                    help="print a one-line human summary instead of JSON (for skill injection)")
...
def main(argv: list[str]) -> int:
    ...
    if args.summary:
        ...
        tail = (": " + " ".join("%s[%s]" % (f["id"], f["severity"] or "?") for f in opens)) if opens else ""
        print("%d open / %d total%s" % (counts["open"], counts["total"], tail))
        return 0
